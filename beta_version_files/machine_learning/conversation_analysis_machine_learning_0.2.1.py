...
def read_csv_file_by_date(filename, start_date, end_date, column_index):
    ...
    with open(filename, "r", encoding="utf-8") as file:
        ...
        for row in reader:
            try:
                row_date = datetime.strptime(row[0], date_format).strftime(
                    "%Y%m%d"
                )  # 날짜 형식을 YYYYMMDD로 통일
                if not first_date:
...
def get_valid_file_path():
    while True:
        filename = input(
            "CSV 또는 TXT 파일 경로를 입력하세요 (예: filename.csv 또는 filename.txt): "
        )
...
if filename.endswith(".csv"):
    ...
    # 대화 내용을 읽어오기
    conversations, first_date, last_date = read_csv_file_by_date(
        filename, start_date, end_date, column_index
    )
    ...
    # 대화 내용을 읽어오기
    conversations, first_date, last_date = read_txt_file_by_date(
