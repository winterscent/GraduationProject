...
combined_tokens = {
    relation: translate.morphs(row) for relation, row in combined_conversations.items()
}
...
def classify_relationship(target_input):
    ...
    for relation, combined_vector in combined_for_vectorize.items():
        d = dist_raw(
            X2[list(combined_for_vectorize.keys()).index(relation)], t_vec[0]
        )  # 대상 문장의 TF-IDF 벡터와의 거리 계산
