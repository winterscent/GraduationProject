...
def classify_relationship(target_input):
    ...
    predicted_scores = {
        inverse_label_dict[i]: score for i, score in enumerate(prediction[0])
    }

    # 최종 점수 비율 25%, 딥러닝 모델 예측 점수 비율 75%로 합산
    combined_scores = {}
    for relation in final_scores.keys():
        combined_scores[relation] = (
            final_scores[relation] * 0.25 + predicted_scores[relation] * 0.75
        )
...
print(
    "\n입력한 대화 내용은 최종 합산 점수에 따라 '{}' 관계에 해당합니다.".format(
        closest_relation_combined
    )
)
